...
def plot_data(distances, times,normalize=False):
    ...
    Args:
        ...
        normaliz: A boolean flag indicating whether the data is normalized
    """

    #Create a new figure with a specified size
    plt.figure(figsize=(8,6))

    #Pltthe daata points as a scatter plot
    plt.plot(distances.numpy(), times.numpy(), color='orange', marker='o', linestyle='none', label='Actual Delivery Times')

    #Check if the daata is normalized to set appropriate labels aand title
    if normalize:
        #Set the plot title for hte normalized dta
        plt.title('Normalized delivery data (Bike & Cars)')
        ...
        plt.grid(True)
        plt.show()
    #Handle the cases for non-normalized data
    else:
...
def plot_final_fit(model, distances, times, distances_norm, times_std, times_mean):
    ...
    #Disable gradient caculation for prediction
    with torch.no_grad():
        #Get predictios from the model using normalized data
        predicted_norm = model(distances_norm)

    #De-normalized the prdictions to their original scal
...
def plot_training_progress(epoch, loss, model, distances_norm, times_norm):
    ...
    #Make prediction using the current state of the model
    predicted_norm = model(distances_norm)
