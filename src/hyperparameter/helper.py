...
color_map, PLOT_SIZE = apply_dlai_style()
mpl.rcParams.update(PLOT_SIZE)
...
def set_seed(seed=42):
    ...
    :param seed: Seeds value to the use default is 42
    """
...
def get_dataset_dataloaders(batch_size=64, subset_size=10_000, imbalanced=False):
    ...
    #Define the image transofrmation pipeline
    ...
    if imbalanced:
        ...
        #Load the standard training dataset, downloading if required
        full_trainset = datasets.CIFAR10(
            root="./cifar10", train=True, download=True, transform=transform
        )

    #Use the full dataset size if a subset size isnot specified
    ...
    #Splitthe subset into training and validation sets
    train_subset, val_subset = random_split(subset, [train_size, val_size])

    #Create a Dataloader for the training set with suffling
...
def plot_metrics_vs_learning_rate(df_metrics):
    ...
    #Iterate through the metrics and plot each one against the leanring rate
    ...
    plt.xscale("log")
    plt.xlabel("Leaning Rate (log scale)")
    plt.ylabel("Metric Value")
    plt.title("Metics vs Leaning Rate")
    plt.legend()
    plt.grid(True)
...
def plot_metrics_vs_batch_size(df_metrics):
    ...
    #Iterate through the metrics and plot each one gainst the batch size
    for metric in ["accuracy", "precision", "recall", "f1_score"]:
        plt.scatter(
...
def plot_results(learning_rates, accuracies):
    ...
    plt.title("Leaning Rate vs Validation Accuracy")
    plt.grid(True)
    plt.show()
...
def train_epoch(model, train_dataloader, optimizer, loss_fun, device, pbar):
    ...
    for batch_idx, (inputs, labels) in enumerate(train_dataloader):
        ...
        #Perform a forward pass to get models outputs
        outputs = model(inputs)
        #calculate the loss
        loss = loss_fun(outputs, labels)
        #Perform a backward pass to compute gradient
        loss.backward()

        #update the model weights
        optimizer.step()

        #Accumulate the loss for epochs
        running_loss += loss.item() * inputs.size(0)

        #Get the predicted class with the heighst score
        _, predicte = outputs.max(1)

        #Update the totle number of samples 
        total +=labels.size(0)

        #Update the number of correctly classified samples
        correct += predicte.eq(labels).sum().item()
...
def train_model(model, optimizer, loss_fcn, train_dataloader, device, n_epochs):
    ...
    for epoch in range(n_epochs):
        ...
        pbar.update_batch(epoch+1)

        #Call the helper function to train the model for one full. epoch
        train_loass, _ = train_epoch(
            model, train_dataloader, optimizer, loss_fcn, device, pbar
        )

        #Log the training loss for the current epoch at a set frequency
        pbar.maybe_log_batch(
            epoch + 1, 
            message=f"Epoch {epoch+1} - Train Loss: {train_loass:.4f}",
        )

    ##Close the progress bar and print the final competion message
    pbar.close("Training completed")